...
const NavItem: React.FC<NavItemProps> = ({ to, isActive, children }) => {
    return (
        <Link href={to} className="inline-block">
            <button
                className={`
                    relative flex w-fit min-w-[140px] justify-center rounded-lg px-4 py-2 text-lg font-semibold tracking-widest text-white
            ...
            >
                <span
                    className={`
                        relative pb-1 after:absolute after:left-0 after:bottom-0 after:h-[2px] after:w-full
                        after:origin-left after:scale-x-0 after:bg-white after:transition-transform after:duration-300
                        ${isActive ? "after:scale-x-100" : "hover:after:scale-x-100 after:bg-white/60"}`}
                >
                    {children}
                </span>
            </button>
