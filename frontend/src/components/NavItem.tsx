...
const NavItem: React.FC<NavItemProps> = ({ to, isActive, children }) => {
  return (
    <Link href={to} className="inline-block">
      <button
        className={`
          relative flex w-fit min-w-[140px] justify-center rounded-lg px-4 py-2
          text-lg font-semibold tracking-widest text-white
      ...
      >
        <span
          className={`
            ...
            ${isActive 
              ? "after:scale-x-100" 
              : "hover:after:scale-x-100 after:bg-white/60"
            }
          `}
        >
          {children}
